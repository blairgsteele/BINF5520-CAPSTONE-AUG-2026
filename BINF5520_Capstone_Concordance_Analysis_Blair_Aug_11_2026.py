...
def load_metadata() -> pd.DataFrame:
    """ Load metadata.csv and filter for samples that actually exist in lineage/ """
    df = pd.read_csv(METADATA_PATH)
    df.columns = df.columns.str.strip().str.lower()
    
    # Keep only samples that have actual output files in lineage/
    existing_accs = [
        p.name.split('.')[0] for p in LINEAGE_DIR.glob("*.pangolin.csv")
    ]
    if existing_accs:
        df = df[df["accession"].astype(str).str.strip().isin(existing_accs)]
